...
- **已学过（本项目过程中建立的概念）**：
  - PWM 原理：频率/占空比/电感平滑，理解为什么高频对电机等效直流
  - 二极管分类：PN（普通整流）、肖特基（低压降/快恢复）、齐纳（稳压/钳位）、TVS（瞬态保护）
  - H 桥续流二极管：8个肖特基，作用是续流+钳压，理解为什么不能用普通PN
  - PN 结物理：耗尽层（无载流子区域）、内建电场、正/反向偏置对耗尽层的影响
  - 击穿机制：雪崩击穿 vs 齐纳隧穿，重掺杂→耗尽层薄→同电压下电场更强
  - 为什么击穿可逆：晶格结构未被破坏，热量才是真正损坏原因
  - 反向恢复时间：少数载流子存储→拖尾电流，肖特基无此问题
  - 耗尽层空间方向：垂直于电流方向，贯穿整个截面，电流无法绕过
  - Saturation（饱和=完全导通）≠ 截止（Cut-off），中英文含义区分
- **需要解释**：
...
- 解释风格：**一句话核心 + 必要时类比**，不堆砌术语，鼓励追问；用户习惯从直觉/疑问出发追问细节，解释要跟着用户的逻辑走
...
> 目标：对每个模块能开口解释"做什么、关键参数怎么定、遇到过什么问题"，而不只是"跑通了"。

### 学习方式

每个大模块拆成若干小知识点，Claude 扮演面试官逐点提问，用户回答，回答不上来再讲解。

### 6 个大模块 + 子模块清单

**模块 1：通信调试 — UART** `uart.c`
- [ ] 1.1 UART 在项目里的角色
- [ ] 1.2 时钟使能（RCC->AHB1ENR / APB1ENR）
- [ ] 1.3 引脚复用配置（MODER + AFR，AF7）
- [ ] 1.4 波特率计算（BRR = 0x16D，APB1=42MHz 怎么来的）
- [ ] 1.5 USART 使能顺序（TE / RE / UE）
- [ ] 1.6 发送：TXE 标志位，为什么要等
- [ ] 1.7 接收：非阻塞 RXNE，ORE 溢出错误处理
- [ ] 1.8 项目坑：ST-Link VCP 失效 → 改 CH340G

**模块 2：信号采集 — ADC** `adc_reg.c`
- [ ] 2.1 两路采集各是什么信号，为什么需要 ADC
- [ ] 2.2 ADC 时钟配置（ADCPRE 分频）
- [ ] 2.3 采样通道配置（SQR / SMPR）
- [ ] 2.4 12bit 结果怎么换算成实际物理量（电压、转速、电流）
- [ ] 2.5 死区处理：adc < 50 → setpoint = 0，为什么要这样做
- [ ] 2.6 16 点均值滤波：原理和代价

**模块 3：转速测量 — 编码器** `encoder_reg.c`
- [ ] 3.1 编码器是什么，输出什么信号
- [ ] 3.2 TIM3 编码器模式：SMS / CCMR1 / CCER
- [ ] 3.3 转速公式推导：delta × 60000 / PPR
- [ ] 3.4 为什么用 TIM3 不用 TIM1/TIM2
- [ ] 3.5 项目坑：编码器不供电 RPM=0，积分飙升

**模块 4：显示 — OLED** `custom_i2c.c` + `OLED_SSD1306.c`
- [ ] 4.1 I2C 协议基础：开始/停止/应答
- [ ] 4.2 I2C 引脚为什么要开漏 + 上拉
- [ ] 4.3 SSD1306 地址 0x3C，怎么确定
- [ ] 4.4 项目坑：OLED 卡死，超时保护怎么加

**模块 5：控制链路 — PID + SPI + FPGA** `pid.c` + `spi_Reg.c` + VHDL
- [ ] 5.1 为什么用串级 PID，单环有什么问题
- [ ] 5.2 位置式离散 PID 公式
- [ ] 5.3 积分限幅（anti-windup）是什么，为什么要加
- [ ] 5.4 内外环参数：kp/ki 怎么定的，整定过程
- [ ] 5.5 SPI 帧格式：2 字节拼 12bit duty，怎么拼
- [ ] 5.6 FPGA PWM：50MHz / 4096 ≈ 12.2kHz，怎么算
- [ ] 5.7 TIM6 1kHz 中断：控制循环执行顺序

**模块 6：参数持久化 — Flash** `w25q64.c`
- [ ] 6.1 为什么要存 Flash，不存会怎样
- [ ] 6.2 W25Q64 写操作流程：WE → 擦除 → 写入
- [ ] 6.3 项目坑：tSHSL 时序违反，静默写入失败

### 当前进度

| 模块 | 状态 |
|------|------|
| 模块 1 UART | ✅ 已完成，笔记存于 `report/interview_prep/module1_uart.md` |
| 模块 2~6 | 待开始 |
