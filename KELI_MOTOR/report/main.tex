...
\begin{table}[H]
    ...
    \begin{tabular}{lll}
        ...
        SPI1     & W25Q64 Flash (verified, ID = 0xEF16) & APB2 5.25\,MHz \\
...
SPI1 is configured at APB2/8 = 5.25\,MHz, Mode 0, 8-bit MSB-first, with
PA4 as a software chip-select. The W25Q64FV (8\,MB NOR Flash, JEDEC ID
\texttt{0xEF16}) is used to store PID parameters across power cycles.

The key operations used are Sector Erase (0x20, 4\,KB, $t_{SE}$ max
400\,ms) and Page Program (0x02, 256\,bytes, $t_{PP}$ max 3\,ms), each
preceded by a Write Enable command (0x06). A critical timing requirement
from the datasheet is:

\begin{quote}
\textbf{tSHSL} (\texttt{/CS} deselect time, erase/program $\to$ read
status register): minimum \textbf{50\,ns}.
\end{quote}

Violating this caused the chip to silently reject all write and erase
commands, with the Write Enable Latch (WEL bit) remaining set as the
diagnostic indicator. The fix was to insert a 1\,ms delay
(\texttt{HAL\_Delay(1)}) after every \texttt{CS\,\textuparrow} before
issuing the next command.

Eight floats (32\,bytes) are stored at sector 0 (\texttt{0x000000}):
a magic number for validity check, followed by the six PID gains
($K_p$, $K_i$, $K_d$ for each loop). On every power-up,
\texttt{PID\_LoadFromFlash()} reads the magic number; if it matches, the
saved parameters are applied automatically without any serial command.
...
\begin{table}[H]
    ...
    \begin{tabular}{p{3cm}p{4cm}p{5.5cm}}
        ...
        \addlinespace
        Flash write silently ignored &
            \texttt{save} reported success but parameters lost on reboot;
            readback returned \texttt{0xFFFFFFFF} &
            tSHSL timing violation: \texttt{/CS} deselect time after Write
            Enable / Erase commands was $\approx$30\,ns, below the 50\,ns
            minimum. Chip discards the command but WEL stays set, masking
            the failure. Fixed by adding 1\,ms delay after each
            \texttt{CS\,\textuparrow}. \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item Non-volatile PID parameter storage via W25Q64 Flash,
          with automatic load on power-up
\end{itemize}

Remaining work includes:
\begin{itemize}
    \item Load testing of the current inner loop under varying mechanical
          load conditions
    \item Re-tuning of PID parameters if load testing reveals instability
\end{itemize}
